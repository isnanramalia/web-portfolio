...
export function RescueTerminal() {
  ...
  return (
    ...
    >
          {/* ── Glass window with doodle style ─────────────────────────── */}
          <div
            className="border-2 border-foreground overflow-hidden bg-card text-card-foreground"
            style={{
              borderRadius: "24px 8px 20px 10px / 10px 20px 8px 24px",
              boxShadow: "6px 6px 0 rgb(var(--foreground) / 0.15)",
              backdropFilter: "blur(12px)",
            }}
          >
            {/* ── Title bar with hand-drawn separator ────────────────────── */}
            <div className="flex items-center gap-3 px-4 py-3 border-b-2 border-foreground bg-muted/30">
              <TrafficLights />
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <Terminal className="w-3.5 h-3.5 text-foreground/60 shrink-0" />
                <span className="font-handwritten text-xs font-bold text-foreground/80 tracking-wide truncate">
                  📟 QA Auto-Heal Terminal — v2.3.1
                </span>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <Minus className="w-3 h-3 text-foreground/30" />
            ...
            <div
              ref={scrollRef}
              className="h-52 sm:h-60 overflow-y-auto p-4 space-y-1.5 scroll-smooth bg-card/60"
              style={{ scrollbarWidth: "none" }}
            >
              {/* Prompt header */}
              <div className="font-mono text-[10px] text-foreground/40 mb-3 pb-2 border-b border-foreground/10">
              ...
              <AnimatePresence>
                {scriptDone && (
                  <motion.div
                    className="flex items-center gap-2 mt-3 pt-3 border-t-2 border-dashed border-emerald-400/40"
                    initial={{ opacity: 0, y: 6 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5 }}
                  >
                    <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0 animate-bounce" />
                    <span className="font-handwritten text-xs text-emerald-600 dark:text-emerald-400 font-bold">
            ...
            {/* ── Progress bar with doodle-style border ──────────────────── */}
            <div className="px-4 pb-4 pt-3 border-t-2 border-foreground bg-muted/20">
              <div className="flex items-center justify-between mb-1.5">
                <span className="font-handwritten text-[10px] font-bold text-foreground/50 tracking-wider">
                  HEALING PROGRESS
                </span>
              ...
              <div className="h-2 rounded-full border border-foreground bg-background overflow-hidden p-0.5">
                <motion.div
